  ...
  const filteredGroupNames = useMemo(() => {
    return Object.keys(groupedProducts).filter((name) => {
      ...
      const matchesCategory =
        selectedCategory === "Todas" || group[0].category === selectedCategory;
      ...
      <div className="bg-white/90 backdrop-blur-xl p-3 md:p-5 rounded-2xl shadow-lg shadow-navy-900/5 border border-white sticky top-[68px] md:top-24 z-40 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
        ...
        <div className="relative w-full sm:w-48" ref={filterRef}>
          <button
            ...
            className="w-full flex items-center justify-between bg-white border border-gray-200 px-4 py-2.5 rounded-xl text-sm font-semibold text-navy-900"
          >
            ...
            <div className="absolute top-full right-0 left-0 mt-2 bg-white rounded-xl shadow-xl border border-gray-100 z-50">
              {[
                ...
                <button
                  ...
                  className="w-full px-4 py-3 text-sm text-left hover:bg-gray-50 flex justify-between"
                >
                  {cat}{" "}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredGroupNames.map((baseName) => {
          ...
          const uniqueVariants = Array.from(
            new Map(variants.map((v) => [v.size || "Estándar", v])).values()
          );

          const selectedVariantId =
            selectedVariants[baseName] || uniqueVariants[0].id;
          const currentProduct =
            uniqueVariants.find((v) => v.id === selectedVariantId) ||
            uniqueVariants[0];

          const colorOptions = currentProduct.colors
            ? currentProduct.colors.split(",").map((c) => c.trim())
            : [];
          const activeColor =
            selectedColors[baseName] ||
            (colorOptions.length > 0 ? colorOptions[0] : "Único");
          ...
          return (
            ...
            >
              <div
                className="relative h-56 cursor-pointer overflow-hidden bg-gray-100"
                onClick={() => {
              ...
              >
                ...
                {isAgotado && (
                  <div className="absolute inset-0 bg-black/40 flex items-center justify-center z-10">
                    <span className="bg-red-600 text-white font-black px-4 py-1 rounded-lg transform -rotate-12">
                      AGOTADO
                    </span>
                  </div>
                )}
              ...
              <div className="p-5 flex flex-col flex-grow">
                <h3 className="font-black text-navy-900 text-lg leading-tight mb-2">
                  {baseName}
                </h3>
                ...
                <div className="space-y-3 mb-4">
                    ...
                    {uniqueVariants.length > 1 ? (
                      <select
                        ...
                        onChange={(e) =>
                          setSelectedVariants({
                            ...selectedVariants,
                            [baseName]: Number(e.target.value),
                          })
                        }
                  ...
                  {colorOptions.length > 0 &&
                    colorOptions[0] !== "Único" &&
                    colorOptions[0] !== "Unicolor" &&
                    !isAgotado && (
                      <div>
                          ...
                          {colorOptions.map((color) => (
                            ...
                            >
                              {activeColor === color && (
                                <Check
                                  size={12}
                                  className={`mx-auto drop-shadow-md ${color === "Blanco" || color === "Rosa" || color === "Gris claro" ? "text-navy-900" : "text-white"}`}
                                />
                              )}
                            </button>
                          ))}
                ...
                <div className="mt-auto flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-navy-900 font-black text-xl">
                  ...
                  {isAgotado ? (
                    <button
                      disabled
                      className="px-4 py-2 bg-gray-100 text-gray-400 rounded-xl font-bold text-xs"
                    >
                      Agotado
                    </button>
                  ) : qty > 0 ? (
                    <div className="flex items-center gap-2 bg-gold-50 border border-gold-200 rounded-full px-1 py-1">
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center text-navy-900 hover:bg-gold-200 rounded-full"
                      >
                        <Minus size={14} />
                      </button>
                      ...
                      <button
                        ...
                        disabled={qty >= currentProduct.stock}
                        className="w-8 h-8 flex items-center justify-center text-navy-900 hover:bg-gold-200 rounded-full"
                      >
                        <Plus size={14} />
                      </button>
                    ...
                    <button
                      ...
                      className="w-10 h-10 rounded-full bg-navy-900 text-white flex items-center justify-center hover:bg-gold-600 transition-colors"
                    >
                      <Plus size={20} />
                    </button>
                  )}
